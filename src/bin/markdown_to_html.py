...
def clean_markdown(markdown_text):
    """Clean the markdown by removing draft notes and image prompt sections."""
    # Remove any "draft" or "note" indicators
    markdown_text = re.sub(r'^Here is a draft .*?format:.*?\n', '', markdown_text, flags=re.IGNORECASE | re.MULTILINE)

    # Remove the image prompt section at the end
    markdown_text = re.sub(r'##\s*Image\s*Prompt.*?$', '', markdown_text, flags=re.DOTALL)

    # Remove any notes about references
    markdown_text = re.sub(r'Note:\s*References.*?need verification.*?\n', '', markdown_text, flags=re.IGNORECASE | re.MULTILINE)

    # Clean up any other artifacts if needed
...
def main():
    ...
    image_prompt = convert_markdown_to_html(
