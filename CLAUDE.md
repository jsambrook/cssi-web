...
TOCICO credentials appear both as visible cards on `/about` (data in `src/data/pages/about.ts`) and as JSON-LD `EducationalOccupationalCredential` entries in `src/data/schema.ts`. These must be kept in sync. See `docs/credentials.md` for the full update procedure.
